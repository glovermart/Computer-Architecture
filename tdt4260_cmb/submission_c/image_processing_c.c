...
void blurIteration(AccurateImage *imageOut, AccurateImage *imageIn, int size) {
	...
	#pragma omp parallel for schedule(auto)                                                 // added this line of code . ADDING PRAGMA....
																								// Interchange variables (loop interchange senterX to sentery AND x to y vice versa)
	for(int senterY = 0; senterY < imageIn->y; senterY++) {  
		...
		for(int senterX = 0; senterX < imageIn->x; senterX++) {
			...
			int countIncluded = 0;
																								// Interchange variables (loop interchange  x to y vice versa)
			for(int y = -size; y <= size; y++) {
																								// putting y computations in y loop and x computations in x loop
				...
				for(int x = -size; x <= size; x++) {
					...
					//if(colourType == 0)
						sum0 += imageIn->data[offsetOfThePixel].red;
					//if(colourType == 1)
						sum1 += imageIn->data[offsetOfThePixel].green;
					//if(colourType == 2)
						sum2 += imageIn->data[offsetOfThePixel].blue;
			...
			double value0 = sum0 / countIncluded;
			...
			//if(colourType == 0)
				imageOut->data[offsetOfThePixel].red = value0;
			//if(colourType == 1)
				imageOut->data[offsetOfThePixel].green = value1;
			//if(colourType == 2)
				imageOut->data[offsetOfThePixel].blue = value2;
		}
...
int main(int argc, char** argv) {
	...
	// Process the tiny case:
		int tinySize = 2;
        blurIteration(imageAccurate2_tiny, imageAccurate1_tiny,  tinySize);
        blurIteration(imageAccurate1_tiny, imageAccurate2_tiny,  tinySize);
        blurIteration(imageAccurate2_tiny, imageAccurate1_tiny,  tinySize);
        blurIteration(imageAccurate1_tiny, imageAccurate2_tiny,  tinySize);
        blurIteration(imageAccurate2_tiny, imageAccurate1_tiny,  tinySize);
	...
	// Process the small case:
		int smallsize = 3;
        blurIteration(imageAccurate2_small, imageAccurate1_small,  smallsize);
        blurIteration(imageAccurate1_small, imageAccurate2_small,  smallsize);
        blurIteration(imageAccurate2_small, imageAccurate1_small,  smallsize);
        blurIteration(imageAccurate1_small, imageAccurate2_small,  smallsize);
        blurIteration(imageAccurate2_small, imageAccurate1_small,  smallsize);
...
//    writePPM("imageAccurate2_tiny.ppm", convertToPPPMImage(imageAccurate2_tiny));
	...
	// Process the medium case:
		...
		int mediumsize = 5;
        blurIteration(imageAccurate2_medium, imageAccurate1_medium,  mediumsize);
        blurIteration(imageAccurate1_medium, imageAccurate2_medium,  mediumsize);
        blurIteration(imageAccurate2_medium, imageAccurate1_medium,  mediumsize);
        blurIteration(imageAccurate1_medium, imageAccurate2_medium,  mediumsize);
        blurIteration(imageAccurate2_medium, imageAccurate1_medium,  mediumsize);
	...
	// Do each color channel
		int largesize = 8;
        blurIteration(imageAccurate2_large, imageAccurate1_large, largesize);
        blurIteration(imageAccurate1_large, imageAccurate2_large, largesize);
        blurIteration(imageAccurate2_large, imageAccurate1_large, largesize);
        blurIteration(imageAccurate1_large, imageAccurate2_large, largesize);
        blurIteration(imageAccurate2_large, imageAccurate1_large, largesize);
